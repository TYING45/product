...
while ($item = $items_result->fetch_assoc()) {
    if ($item['Seller_ID'] !== $current_seller_id) {
        continue; // 非本賣家商品跳過
    }
...
foreach ($products as $p) {
    $seller_total += $p['subtotal'];
}

$shipping_fee = $order['shipping_fee'] ?? 0;
$seller_total_with_shipping = $seller_total + $shipping_fee;

$total_amount = $order['total_price'] + $shipping_fee; // 全訂單總價，保留不動
?>
...
<main>
    ...
    <div class="order-container">
        ...
        <tr>
           ...
           <tr>
                <td class="label1">賣家總金額</td>
                <td class="content1" colspan="5"><?= htmlspecialchars(number_format($seller_total_with_shipping, 2)) ?></td>
        </tr>
        </table>
    </div>
        ...
        <form method="post" action="UPdateOrder.php?id=<?= htmlspecialchars($order_display_id) ?>">
            <table>
                ...
                <tr>
                    ...
                    <td class="content"><?= htmlspecialchars($order['Payment_status'] ?? '尚未付款') ?></td>
    ...
    <div class="button-container">
        <input type="button" value="返回表單" onclick="location.href='Seller_Order.php'">
    </div>
</main>
