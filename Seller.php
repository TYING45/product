...
// 計算總筆數
$total_query = "SELECT COUNT(*) FROM `seller`" . $where;
...
// 取得商品資料（加入分頁）
$query = "SELECT * FROM `seller` " . $where . " ORDER BY `Seller_ID` DESC LIMIT ?, ?";
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['id']) && isset($_POST['stock'])) {
    $id = intval($_POST['id']);       // 獲取商品ID
    $stock = intval($_POST['stock']); // 獲取新的庫存數量
    }
...
<!-- id  權限管理 -->
    ...
    <input name="keyword" type="text" id="keyword" placeholder="請輸入關鍵字(ID、聯絡人)" size="12" 
        value="<?php echo isset($_GET['keyword']) ? htmlspecialchars($_GET['keyword']) : ''; ?>">
    <input type="submit" name="import" id="button0" value="查詢">
    </form>
        <input type="button" onclick="location.href='CSV_Seller.php'" value="輸出CSV">
        <input type="button" onclick="location.href='Upload_Seller.html'" value="上傳資料">
    </form>
    <table border="1">
        <tr>
            <th>賣家ID</th>
    ...
    <tr>
        ...
        if ($num_pages > ($range + 1)) {
            echo "<a href='Seller.php?page=1" . keepURL() . "'>1</a> ";
        ...
        for ($i = max(1, $num_pages - $range); $i <= min($total_pages, $num_pages + $range); $i++) {
            if ($i == $num_pages) {
                ...
                echo "<a href='Seller.php?page={$i}" . keepURL() . "'>$i</a> ";
            }
        }
        ...
        if ($num_pages < ($total_pages - $range)) {
            ...
            echo "<a href='Seller.php?page={$total_pages}" . keepURL() . "'>$total_pages</a>";
        }
        ?>
    </tr>
</table>
